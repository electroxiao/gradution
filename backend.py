...
# --- 步骤 1: 意图识别 (通用) ---
# 无论是老师输入批量错题，还是学生提问，都需要提取其中的 Java 实体
def extract_keywords_with_llm(client, user_input, history=[]):
    print(f"\n🧠 [Step 1] 正在分析输入内容 (模式: 通用)...")
    ...
    2. **去噪**：忽略无关的描述性文字（如“学生A错了”、“这道题很难”），只保留技术术语。
    3. **指代还原**：结合上下文 {context_str} 还原代词。
    4. **格式**：只返回 JSON 列表，如 ["ArrayList", "IndexOutOfBoundsException", "数组"]。
    """
...
# --- 步骤 3: 核心修改 - 双模式生成 ---
def ask_deepseek_stream(client, user_input, context_knowledge, history=[], mode="student"):
    """
    mode: "student" (默认) 或 "teacher"
    """
    print(f"\n💬 [Step 3] AI 正在思考 (当前模式: {mode}) ...")
    ...
    if mode == "student":
        ...
        === 回复示例 ===
        "同学，这里报了空指针。根据图谱分析，这说明你对 **[引用类型]** 的初始化机制理解不够。我想问问你，在 Java 里，如果不给一个对象赋值，它默认是什么？"
        """
    ...
    # 教师模式下，通常不需要太长的历史记录，主要关注当前的批量输入
    # 学生模式下，需要保留上下文
