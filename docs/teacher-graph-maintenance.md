...
- ChatPage 默认在回答前调用聊天 RAG：后端从当前问题和最近上下文中抽取图谱实体关键词，查询 Neo4j 两跳子图并整理种子节点、相关节点、路径证据和检索摘要，前端展示相关节点和检索过程。
- ChatPage 在聊天回答完成后仍会异步抽取本轮涉及的知识点，写入提问记录用于学生回看和教师热点统计。
- 聊天提问记录只描述学生问过哪些知识点，不会写入 `user_weak_points`，也不会改变 `user_knowledge_states`。
