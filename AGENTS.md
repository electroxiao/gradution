...
面向论文正文、章节重写、答辩稿和表格说明时遵守以下限制：

- 禁止使用“不是……而是……”“并不……而是……”“不等于……只有……才……”等否定式对比句式。
- 禁止使用“需要注意的是”“请特别注意”“值得强调的是”“切记”等无信息量提醒。
- 禁止在正文中用冒号引出“例如”“比如”“包括”等说明段落。
- 禁止给普通术语加引号，也不要用引号表示强调或所谓的概念。
- 禁止堆砌“从而”“进而”“以此”“借此”“据此”“基于此”“换言之”“也就是说”“换句话说”等虚词连接。
- 禁止使用“尽管……但是……”“虽然……然而……”“如果……那么……”等复杂条件或让步结构，必要条件可保留。
- 禁止使用“综上所述”“总而言之”“一言以蔽之”“经过上述分析可知”等总结套话。
- 禁止使用“既……又……”“一方面……另一方面……”“不仅……而且……”等刻意对称结构。
- 禁止使用“我们”“人们”“大家”“相关研究人员”等模糊泛指主语。
- 禁止使用“值得注意的是”“有趣的是”“令人惊讶的是”“显而易见的是”等评价性插入语。
- 同一段中最多使用一次“系统”或“本系统”作为主语，避免连续多个句子都以“系统”开头。
...
- 学生端聊天问答、知识图谱增强回答、提问知识点记录、作业错题薄弱点识别、弱点推荐图谱、针对性训练
- 教师端图谱维护、学生与班级管理
...
维护学生聊天 RAG 或教师图谱相关逻辑前，先阅读：

- `docs/chat-rag-flow.md`
...
- 教师图谱画布由 `frontend/src/components/KnowledgeGraphCanvas.vue` 基于 `@neo4j-nvl/*` 渲染。
- TeacherGraphPage 维护 Neo4j 图谱。

## 业务边界

- 系统图谱数据以 Neo4j 图谱为准，用于聊天 RAG 检索、聊天提问知识点记录、弱点推荐、图谱展示、作业题目知识点绑定和训练节点上下文。
- 学生端聊天默认启用图谱检索：后端先抽取图谱实体关键词、查询 Neo4j 两跳子图并整理种子节点、相关节点、路径证据和检索摘要，再用检索事实生成图谱增强回答；前端通过 `graph_trace` 展示相关节点和检索过程。
- 聊天 RAG 的 `facts`、`reasoning_trace`、`retrieval_trace` 随助手消息保存在 `chat_messages`，用于历史回显；聊天提问知识点事件仍由回答完成后的后台抽取写入 `chat_knowledge_events`。
- 聊天 RAG 和聊天提问记录都不直接写入 `user_weak_points` 或 `user_knowledge_states`。
...
- 涉及聊天 RAG、`graph_trace`、`facts_json`、`reasoning_trace_json` 或 `retrieval_trace_json` 时，阅读 `docs/chat-rag-flow.md` 后再改。
